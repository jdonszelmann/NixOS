...
let
  ...
  matrix-registration-cli-wrapper = pkgs.stdenv.mkDerivation {
    ...
    buildCommand = ''
      mkdir -p $out/bin
      makeWrapper ${pkgs.donsz.matrix-registration}/bin/matrix-registration "$out/bin/matrix-registration" \
  ...
  synapse_port = (lists.findFirst
    ...
    config.services.matrix-synapse.settings.listeners).port;
      ...
      type = types.submodule {
        ...
        options = {
          ...
          client_logo = mkOption {
            ...
            default =
              "${pkgs.donsz.matrix-registration}/lib/python3.8/site-packages/matrix_registration/static/images/element-logo.png";
          };
          db = mkOption {
            type = types.str;
  ...
  config = mkIf cfg.enable {
    systemd.services.matrix-registration = {
      ...
      serviceConfig = {
        ...
        PrivateTmp = true;
        WorkingDirectory =
          pkgs.donsz.matrix-registration; # necessary for the database migration scripts to be found
        ...
        ExecStart = ''
          ${pkgs.donsz.matrix-registration}/bin/matrix-registration \
